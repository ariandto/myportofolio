...
const Projects: React.FC<{ t: any }> = ({ t }) => {
  return (
    <section id="main-element" className="py-40 px-4 sm:px-8">
      <h1
        data-aos="fade-down"
        ...
        {projectList.map((data: ProjectItem, index) => {
          ...
          return (
            <div
              key={index}
              data-aos="fade-up"
              className="md:grid md:grid-cols-2 items-center gap-8 rounded-2xl border border-blue-200/60 p-8 shadow-lg bg-transparent backdrop-blur-sm hover:shadow-xl transition-all duration-300 hover:border-blue-300/80"
            >
              {/* GAMBAR / SLIDER */}
              <div
                data-aos="fade-right"
                className={`w-full flex-[1.2] cursor-pointer select-none rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 ${
              ...
              {/* DESKRIPSI */}
              <div data-aos="fade-left" className="flex-[0.8] space-y-6 text-center">
                <h2
                ...
                <div className="flex select-none justify-center gap-4 pt-4 flex-wrap">
                  <button
                    onClick={() =>
                      data.linkDemo
